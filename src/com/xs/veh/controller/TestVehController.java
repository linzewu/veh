...
import com.xs.veh.util.FtpUtils;
@Controller
...
public class TestVehController {
	...
	public @ResponseBody String zjRecordAdd(String lsh) {
...
//		}
	...
	@UserOperation(code="photoUppload",name="动车综检联网")
	...
	public @ResponseBody String photoUppload(String lsh) throws IOException {
		...
		String fileName = "";
		if(zq45 != null) {
			fileName = getFileName("wj1",testVeh.getBgdbh());
		...
		if(yh45 != null) {
			fileName = getFileName("wj2",testVeh.getBgdbh());
		...
		if(cjh != null) {
			fileName = getFileName("wj3",testVeh.getBgdbh());
		...
		InputStream zdgwzp = zhCheckDataManager.getIamge(lsh, "0322");
		if(zdgwzp != null) {
			fileName = getFileName("zd",testVeh.getBgdbh());
			FtpUtils.uploadFile("", fileName, zdgwzp, ftpClient);
		}
		////灯光检测
		InputStream dggwzp = zhCheckDataManager.getIamge(lsh, "0321");
		if(dggwzp != null) {
			fileName = getFileName("dg",testVeh.getBgdbh());
			FtpUtils.uploadFile("", fileName, dggwzp, ftpClient);
		}
		//动力性
		InputStream dlxjygwzp = zhCheckDataManager.getIamge(lsh, "0999");
		if(dlxjygwzp != null) {
			fileName = getFileName("dl",testVeh.getBgdbh());
		...
		if(dp != null) {
			fileName = getFileName("dp",testVeh.getBgdbh());
		...
		if(xsz != null) {
			fileName = getFileName("xsz",testVeh.getBgdbh());
	...
	public String getFileName(String fix,String bgdbh) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
		String dateStr = sdf.format(new Date());
		return fix+"_"+dateStr+"_"+bgdbh;
	}
