...
public class DeviceBrakRollerDriverOfJXGT extends AbstractDeviceBrakRoller {

	private static Logger logger = Logger.getLogger(DeviceBrakRollerDriverOfJXGT.class);
	...
	public BrakRollerData startCheck(VehFlow vehFlow) throws SystemException, IOException, InterruptedException {
		...
		try {
			...
			String zw = vehFlow.getJyxm().substring(1,2);
			...
			deviceBrakRoller.getDisplay().sendMessage(zw +"轴制动请到位", DeviceDisplay.XP);
			...
			while (true) {
				if (deviceBrakRoller.getSignal()) {
					deviceBrakRoller.getDisplay().sendMessage(vehFlow.getHphm(), DeviceDisplay.SP);
					deviceBrakRoller.getDisplay().sendMessage(zw + "轴已到位", DeviceDisplay.XP);
					i++;
				} else {
					deviceBrakRoller.getDisplay().sendMessage(vehFlow.getHphm(), DeviceDisplay.SP);
					deviceBrakRoller.getDisplay().sendMessage(zw+"轴制动请到位", DeviceDisplay.XP);
			...
			checkingFlage = false;
			deviceBrakRoller.getDisplay().sendMessage(vehFlow.getHphm(), DeviceDisplay.SP);
			deviceBrakRoller.getDisplay().sendMessage(zw+"轴制动检测完成", DeviceDisplay.XP);
			logger.info("检测结束");
